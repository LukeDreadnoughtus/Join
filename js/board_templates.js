...
if (taskData.assignedUsers.length === 0) {
   assignedUsersHtml = `<div class="no_assignies"><p>No assigned</p>
  <p>Users yet</p></div>`;
} else {
 ...
 if (taskData.subtask && typeof taskData.subtask === "object") {
  ...
  subtasksHtml = subtaskEntries
    .map(([key, currentSubtask]) => `
      ...
      <input 
      type="checkbox" 
      id="subtask_${key}" ${currentSubtask.done ? "checked" : ""}
      onclick="toggleSubtask('${key}', ${taskData})">
      <label for="subtask_${key}" class="subtask_label">
