...
function addSubtask(text) {
  if (!text) return;
  const item = document.createElement("div"), left = document.createElement("div"), span = document.createElement("span"), dot = document.createElement("div"), buttons = document.createElement("div");
  item.className = "subtask-item"; left.className = "subtask-left"; span.className = "subtask-text"; dot.className = "subtask-dot"; buttons.className = "subtask-buttons";
  span.textContent = text; left.append(dot, span);
  const editBtn = document.createElement("button_edit"), delBtn = document.createElement("button_delete"); editBtn.innerHTML = `<svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
...
</svg>`; delBtn.innerHTML = `<svg width="16" height="18" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`;
  editBtn.onclick = () => makeSubtaskEditable(item, span, buttons, editBtn, delBtn); delBtn.onclick = () => item.remove();
  buttons.append(editBtn, delBtn); item.append(left, buttons); document.getElementById("subtask-list").appendChild(item);
}

function makeSubtaskEditable(item, span, buttons) {
  const left = item.querySelector(".subtask-left");
  const inputEdit = document.createElement("input");
  inputEdit.type = "text"; inputEdit.value = span.textContent; inputEdit.className = "edit-input";
  left.innerHTML = ""; left.appendChild(inputEdit);
  buttons.style.display = "none";
  const wrapper = document.createElement("div");
  wrapper.className = "subtask-input-wrapper"; wrapper.appendChild(inputEdit);
  const check = document.getElementById("subtask-add").cloneNode(true);
  const cancel = document.getElementById("subtask-clear").cloneNode(true);
  check.classList.remove("hidden"); cancel.classList.remove("hidden");

  check.onclick = () => {
    span.textContent = inputEdit.value.trim() || span.textContent;
    left.innerHTML = ""; left.appendChild(span);
